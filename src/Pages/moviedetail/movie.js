...
const Movie = () => {
    ...
    const getData = () => {
        fetch(`https://api.themoviedb.org/3/movie/${id}?api_key=d8142d6f48f6c7a3cd27705d8097244a&language=en-US`)
        .then(res => res.json())
                        ...
                        <div className="movie__genres">
                            {
                                ...
                                currentMovieDetail.genres.map((genre,index) => (
                                    ...
                                    <span key={genre.id || index} className="movie__genre" id={genre.id}>{genre.name}</span>
                                ))
                                : 
                                ""
                            }
            ...
            <div className="movie__production">
                {
                    currentMovieDetail && currentMovieDetail.production_companies && currentMovieDetail.production_companies.map((company,index) => (
                                ...
                                company.logo_path 
                                && 
                                (<span key={company.id || index} className="productionCompanyImage">
